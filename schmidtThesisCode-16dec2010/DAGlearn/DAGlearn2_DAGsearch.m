function [adj,hashTable,scoreTrace,evalTrace,adjTrace] = DAGlearn2_DAGsearch(X,scoreType,SC,adj,A,maxEvals,...
  hashTable, verbose, maxIter, maxFanIn)

if nargin < 8, verbose = false; end
if nargin < 9, maxIter = maxEvals; end
if nargin < 10, maxFanIn = inf; end
...
evals = 0;
if verbose >= 2, fprintf('Evaluating initial graph\n'); end
for c = 1:nNodes
...
if verbose >= 2, fprintf('Evaluating all legal moves\n'); end
iter = 1;
while iter<maxIter
  if verbose >= 2, fprintf('iter %d of %d\n', iter, maxIter); end
  iter = iter+1;
    minScore = 0;
    moveType = 0;
    prevMoveType = -1;
            ...
            if rev(p,c) == 0 && SC(c,p) == 1
               ...
               for t=size(prevrev, 1):-1:1
                 if isequal(prevrev(t,1:2), [p c])
                   if verbose >= 2, fprintf('tried reversing %d->%d in step %d\n', p, c, prevrev(t,3)); end
                   valid = false;
                   break;
                 end
               end
               if valid
                 ...
                 if score_del - score(c) + score_rev - score(p) < minScore
                   ...
                   moveType = REVERSAL;
                   move = [p c];
                   prevrev(end+1,:) = [p c iter];
                 end
               end
            end
        elseif anc(c,p) == 0 && sum(adj(:,c))<maxFanIn
