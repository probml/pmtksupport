function [x, objective, distance, times, mses] = ...
...
for outer = 1:maxiter
    ...
    while violations
       ...
       viol = norm(z-x,inf);
       prev_viol = norm(zprev-xprev,inf);
   ...
   objective(outer+1) = 0.5*(resid(:)'*resid(:)) + tau*phi_function(x);
    ...
    if (outer>1)
        ...
        switch stopCriterion
         ...
         if ( criterion(outer) <= tolA )
             if verbose
                 fprintf('iter = %d, obj = %3.3g, ||x-z|| = %g, stop criterion = %3.3g, ( target = %3.3g )\n', outer, objective(outer+1), distance(outer), criterion(outer), tolA)
    ...
    if verbose
        ...
        fprintf('iter = %d, obj = %3.3g, ||x-z|| = %3.3g, stop criterion = %3.3g, ( target = %3.3g )\n', outer, objective(outer+1), distance(outer), criterion(outer), tolA)
    end
    xprev = x;
    zprev = z;
