...
%
% See also:
%   EIMM_UPDATE, EIMM_SMOOTH, EIMM_FILTER
...
function [X_i,P_i,MU,X,P] = eimm_filter(X_ip,P_ip,MU_ip,p_ij,ind,dims,A,a,Q,Y,H,h,R)
    % Number of models
    m = length(X_ip);
    ...
    for i = 1:m
        ...
        [X_p{i}, P_p{i}] = ekf_predict1(X_0j{i}(ind{i}),P_0j{i}(ind{i},ind{i}),A{i},Q{i},a{i});
        %[X_p{i}, P_p{i}] = kf_predict(X_0j{i}(ind{i}),P_0j{i}(ind{i},ind{i}),A{i},Q{i});

        % Update the estimates
        [X_i{i}, P_i{i}, K, IM, IS] = ekf_update1(X_p{i},P_p{i},Y,H{i},R{i},h{i});
